...
void CQ_close (CQhandle *h) { /* uzavre handle na frontu */
  ...
  if (h->rdwr == 0) {
    ...
    if (__sync_add_and_fetch (&q->openwriters, -1) == 0) {
      pthread_mutex_lock (q->mutex); // lock object
      q->eof = 1;
      pthread_cond_broadcast (q->notEmpty); // inform waiting readers
      pthread_mutex_unlock (q->mutex);      // lock object
    }
...
char *Wn_getBuffer (CQhandle *h) { /* doda volny buffer pro zapis zpravy */
  ...
  while ((old = q->allocated) - q->deallocated == q->queuesize) {
    pthread_cond_wait (q->notFull, q->mutex); // wait for a free entry
  }
  index = old & q->mask;
  e = q->queue + index;
  b = e->buffer;
...
char *Wn_getBufferNW (CQhandle *h) { /* doda volny buffer pro zapis zpravy, neni-li, neceka */
  ...
  q = h->q;
  pthread_mutex_lock (q->mutex); // lock object
  if ((old = q->allocated) - q->deallocated == q->queuesize) {
    pthread_mutex_unlock (q->mutex); // release lock
    return 0;
  }
  index = old & q->mask;
...
char *Rn1_getMsg (CQhandle *h) { /* doda buffer se zpravou */
  ...
  q = h->q;
  pthread_mutex_lock (q->mutex);	 // lock object
  while (((old = q->tail) == q->head)) { // && (!q->eof)) {
    if (q->eof) {
      pthread_mutex_unlock (q->mutex); // release lock
      return 0;
    }
    pthread_cond_wait (q->notEmpty, q->mutex); // wait for a message
  }
  index = old & q->mask;
  e = q->queue + index;
  b = e->buffer;
